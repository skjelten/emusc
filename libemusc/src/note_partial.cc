    ...
    _pcmRom(pcmRom)
{
  ...
  double instPartPitchTune =
    exp(((_ctrlRom.instrument(_instrument).partials[0].coarsePitch - 0x40)* 100
	 + (_ctrlRom.instrument(_instrument).partials[0].finePitch - 0x40))
	* log(2) / 1200);

  // Calculate fine pitch offset from sample definition
  double samplePitchTune =
    exp((_ctrlRom.sample(_sampleIndex).pitch - 1024) / 16 * log(2) / 1200);
