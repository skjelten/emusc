    ...
    _stream(NULL)
{
  ...
  if (!pa_sample_spec_valid(&_sampleSpec))
    throw (QString("Pulse error: Sample spec invalid"));

  // Set up a new main loop
  if (!(_mainLoop = pa_mainloop_new()))
    throw (QString("Pulse error: pa_mainloop_new() failed"));
...
//    throw (Ex(-1, "Pulse error: pa_signal_init() failed"));
  ...
  if (!(_context = pa_context_new(_mainLoopApi, "EmuSC")))
    throw (QString("Pulse error: pa_context_new() failed"));
  ...
  if (pa_context_connect(_context, NULL, f, NULL) < 0)
    throw (QString("Pulse error: pa_context_connect() failed"));
...
void AudioOutputPulse::start(void)
{
  _quit = false;
  _audioOutputThread = new std::thread(&AudioOutputPulse::run, this);  
}
