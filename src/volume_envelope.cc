    ...
    _finished(false)
{
  ...
  // FIXME: Why do we have to divide by 2 to (almost) match the real hardware?
  _phaseDuration[5] = (_convert_time_to_sec(instPartial.TVALenP5 & 0x7F) / 2);
  _phaseDuration[5] *= (instPartial.TVALenP5 & 0x80) ? 2 : 1;
  if ((instPartial.TVALenP5 & 0x80))
